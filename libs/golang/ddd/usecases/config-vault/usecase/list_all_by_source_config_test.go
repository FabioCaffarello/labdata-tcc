...
type ListAllBySourceConfigUseCaseSuite struct {
	suite.Suite
	repoMock *mockrepository.ConfigRepositoryMock
	useCase  *ListAllBySourceConfigUseCase
}

func TestListAllBySourceConfigUseCaseSuite(t *testing.T) {
	suite.Run(t, new(ListAllBySourceConfigUseCaseSuite))
}

func (suite *ListAllBySourceConfigUseCaseSuite) SetupTest() {
	suite.repoMock = new(mockrepository.ConfigRepositoryMock)
	suite.useCase = NewListAllBySourceConfigUseCase(suite.repoMock)
}

func (suite *ListAllBySourceConfigUseCaseSuite) TestExecuteWhenSuccess() {
	...
	suite.repoMock.On("FindAllBySource", "source1").Return(entityConfigs, nil)
	...
	output, err := suite.useCase.Execute("source1")
...
func (suite *ListAllBySourceConfigUseCaseSuite) TestExecuteWhenError() {
	suite.repoMock.On("FindAllBySource", "source1").Return(nil, fmt.Errorf("error"))
